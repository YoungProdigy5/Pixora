...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function saveBlob(blob, baseName, extension) {
    ...
    /* =====================================================
       DOWNLOAD OPTIONS (ASPECT RATIO)
       ===================================================== */

    const DOWNLOAD_RATIOS = [
        { id: "original", title: "Original", note: "Keep as uploaded" },
        { id: "9:16", title: "9:16", note: "Phone" },
        { id: "9:19.5", title: "9:19.5", note: "Tall phone" },
        { id: "16:9", title: "16:9", note: "Landscape" },
        { id: "1:1", title: "1:1", note: "Square" },
        { id: "4:5", title: "4:5", note: "Portrait" }
    ];

    let downloadItem = null;
    let downloadRatio = "original";
    let downloadModal = null;
    let naturalRatio = 9 / 16;

    function ratioValue(id) {
        if (id === "original") return naturalRatio;
    ...
    function updateDownloadPreview() {
        const box = $("pixoraPreviewBox");

        if (!box) return;

        const ratio = ratioValue(downloadRatio);

        box.style.aspectRatio = String(ratio);
        box.style.maxWidth = Math.round(210 * ratio) + "px";
    }
    ...
    function buildDownloadModal() {
        ...
        downloadModal.className = "pixora-download-modal";
        downloadModal.hidden = true;

        downloadModal.innerHTML = `
            <div class="pixora-download-backdrop" data-close></div>

            <div class="pixora-download-panel" role="dialog" aria-modal="true"
                 aria-labelledby="pixoraDownloadTitle">

                <button type="button" class="pixora-download-close" data-close
                        aria-label="Close download options">×</button>

                <div class="pixora-download-heading">
                    <span class="pixora-download-label">PIXORA</span>
                    <h2 id="pixoraDownloadTitle">Download wallpaper</h2>
                    <p>Pick the shape that fits your screen. The image is cropped from the center.</p>
                </div>

                <div id="pixoraPreviewBox"
                     style="width:100%;margin:0 auto;overflow:hidden;border-radius:14px;background:#000;">
                    <img id="pixoraPreviewImg" alt=""
                         style="display:block;width:100%;height:100%;object-fit:cover;">
                </div>

                <div class="pixora-download-group">
                    <h3>Aspect ratio</h3>
                    <div class="pixora-option-grid" id="pixoraRatioGrid"></div>
                </div>

                <button type="button" id="pixoraDownloadGo" class="pixora-download-main">
                    Download wallpaper
                </button>

                <p id="pixoraDownloadStatus" class="pixora-download-status"></p>
            </div>
        `;

        document.body.appendChild(downloadModal);

        const grid = downloadModal.querySelector("#pixoraRatioGrid");

        DOWNLOAD_RATIOS.forEach(option => {
            const button = document.createElement("button");
            button.type = "button";
            button.className = "pixora-option";
            button.dataset.ratio = option.id;
            button.innerHTML =
        ...
        grid.addEventListener("click", event => {
            ...
            downloadRatio = button.dataset.ratio;

            grid.querySelectorAll(".pixora-option").forEach(option => {
        ...
        downloadModal
            .querySelector("#pixoraDownloadGo")
            .addEventListener("click", startWallpaperDownload);

        downloadModal
            .querySelector("#pixoraPreviewImg")
    ...
    function openDownloadModal(item) {
        const modal = buildDownloadModal();

        downloadItem = item;
        downloadRatio = "original";

        modal.querySelectorAll(".pixora-option").forEach(option => {
            option.classList.toggle("active", option.dataset.ratio === "original");
        });

        modal.querySelector("#pixoraDownloadStatus").textContent = "";

        const button = modal.querySelector("#pixoraDownloadGo");
        button.disabled = false;
        button.textContent = "Download wallpaper";

        modal.querySelector("#pixoraPreviewImg").src = imageUrl(item);

        updateDownloadPreview();

        modal.hidden = false;
        document.body.classList.add("pixora-modal-open");
    }
    ...
    function closeDownloadModal() {
        ...
        downloadModal.hidden = true;
        document.body.classList.remove("pixora-modal-open");

        downloadItem = null;
    }

    async function cropImageBlob(blob, ratioId) {
        const [ratioWidth, ratioHeight] = ratioId.split(":").map(Number);
    ...
    async function startWallpaperDownload() {
        const item = downloadItem;

        if (!item || !downloadModal) return;
        ...
        const button = downloadModal.querySelector("#pixoraDownloadGo");
        const status = downloadModal.querySelector("#pixoraDownloadStatus");
        const title = item.title || "PIXORA-wallpaper";
        ...
        try {
            ...
            if (downloadRatio === "original") {
                await saveFile(url, title, "jpg");
            } else {
                const response = await fetch(url);
                ...
                const cropped = await cropImageBlob(original, downloadRatio);

                const ratioName = downloadRatio.replace(":", "x").replace(".", "-");

                saveBlob(
                    cropped,
                    title + "-" + ratioName,
                    cropped.type === "image/png" ? "png" : "jpg"
                );
            }

            status.textContent = "Download started.";

            setTimeout(closeDownloadModal, 900);
        } catch (error) {
            console.error("PIXORA download:", error);
            status.textContent = "Could not prepare this size. Try Original.";
        } finally {
            button.disabled = false;
            button.textContent = "Download wallpaper";
        }
